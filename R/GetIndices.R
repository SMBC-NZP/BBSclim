...
GetIndices <- function(alpha){
  ...
  if(!is.na(pass)){
    ...
    avg.lat <- dplyr::summarise(prob_grp, value = sum(lat * Prob, na.rm = TRUE)/sum(Prob, na.rm = TRUE))
    avg.lat$sd.err <- delta(index = "avg.lat", betas = betas, est = avg.lat$value, alpha)
    avg.lat$ind <- "avg.lat"

    avg.lon <- dplyr::summarise(prob_grp, value = sum(lon * Prob, na.rm = TRUE)/sum(Prob, na.rm = TRUE))
...
delta <- function(index, est, betas, alpha, epslon = 0.1e-10) {
    ...
    grad <- matrix(0, length(est), length(all.betas))
      ...
      for (jj in 1:length(all.betas)) {
        ...
        if(index == "avg.lat"){
          est2 <- dplyr::summarise(prob_grp2, avg.lat = sum(lat * Prob, na.rm = TRUE)/sum(Prob, na.rm = TRUE))$avg.lat
        }

        if(index == "avg.lon"){
          est2 <- dplyr::summarise(prob_grp2, avg.lon = sum(lon * Prob, na.rm = TRUE)/sum(Prob, na.rm = TRUE))$avg.lon
        }
