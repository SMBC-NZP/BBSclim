...
mod_eval <- function(mod, dig = 3, large = 8, time, het){
  jj <- grep('std.error', mod)
  jj2 <- grep('Individual Site estimates of <psi>', mod)

  betas <- mod[(jj + 1):(jj2 - 1)]
  coefs <- as.numeric(substr(betas, 41, 50))
  std.er <- as.numeric(substr(betas, 54, 63))

  num.betas <- plyr::ldply(psi_mods[[i]], function(x) length(x))$V1
  ...
  if(num.betas[1] > 1) {
    ...
    psi.cov	<- drop.bad.coef(psi.keep, mods[[i]]$psi.cov)
  }
  if(num.betas[2] > 1) {
    th0.keep 	<- !is.na(std.er[(2 + num.betas[1]):sum(num.betas[1:2])])
    th0.cov	<- drop.bad.coef(th0.keep, mods[[i]]$th0.cov)
  }
  if(num.betas[3] > 1) {
    th1.keep 	<- !is.na(std.er[(2+sum(num.betas[1:2])):sum(num.betas[1:3])])
    th1.cov	<- drop.bad.coef(th1.keep, mods[[i]]$th1.cov)
  }
  if(num.betas[4] > 1) {
    gam.keep 	<- !is.na(std.er[(2+sum(num.betas[1:3])):sum(num.betas[1:4])])
    gam.cov	<- drop.bad.coef(gam.keep, mods[[i]]$gam.cov)
  }
  if(num.betas[5] > 1) {
    eps.keep 	<- !is.na(std.er[(2+sum(num.betas[1:4])):sum(num.betas[1:5])])
    eps.cov	<- drop.bad.coef(eps.keep, mods[[i]]$eps.cov)
  }
  if(num.betas[6] > 1) {
    p1.keep <- p2.keep  <- !is.na(std.er[(2+sum(num.betas[1:5])):sum(num.betas[1:6])])
    if(het)      p2.keep  <- !is.na(std.er[(2+sum(num.betas[1:6])):(sum(num.betas[1:6])+num.betas[6])])
    if(time){
      p1.cov	<- drop.bad.coef(p1.keep & p2.keep, c(seq(1:pao$nseasons) * pao$nsurveyseason[1] - pao$nsurveyseason[1] + 1, mods[[i]]$p1.cov))
    }else{
      p1.cov	<- drop.bad.coef(p1.keep & p2.keep, mods[[i]]$p1.cov)
    }
  }
  ...
  if (mean(c(psi.keep, th0.keep, th1.keep, gam.keep, eps.keep, p1.keep, p2.keep, 1))==1) {
    ...
    if(num.betas[6]>1) {
      ...
      if(het)	p2.keep   	<- abs(std.er[(2+sum(num.betas[1:6])):(sum(num.betas[1:6])+num.betas[6])])<large
      if(time){
        p1.cov	<- drop.bad.coef(p1.keep & p2.keep, c(seq(1:pao$nseasons) * pao$nsurveyseason[1] - pao$nsurveyseason[1] + 1, mods[[i]]$p1.cov))
      }else{
        p1.cov	<- drop.bad.coef(p1.keep & p2.keep, mods[[i]]$p1.cov)
  ...
  jj <- grep('significant digits', mod)
  num.conv <- mod[jj]
