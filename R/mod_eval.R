...
mod_eval <- function(pres_out, mod, pao2, psi = FALSE, dig = 2, large = 8, is.annual, is.het,
  ...
  if(psi){ # Check that psi parameters are estimable
    ...
    if(num.betas[1] > 1) {
      ...
      psi.check[quads.coord - 1] <- coefs[quads.coord]/std.er[quads.coord] < 0
      if(strict.psi){
        psi.check[quads.clim - 1] <- coefs[quads.clim]/std.er[quads.clim] < 0
      }else{
        ...
        if(mean(c(psi.check, th0.check, th1.check, gam.check, eps.check, p1.check, p2.check, 1))==1) {
          ...
          if(num.betas[4] > 1) {
            ...
            if(strict.gam){
              ...
              gam.check[quads - 4 - sum(num.betas[1:3])]	<- coefs[quads] < 1.96
            }
          }
          if(num.betas[5] > 1) {
            ...
            if(strict.gam){
              ...
              eps.check[quads - 5 - sum(num.betas[1:4])]	<- coefs[quads] > 1.96
            }
          }
        }
      }
